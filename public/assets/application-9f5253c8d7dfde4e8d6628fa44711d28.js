...
(function() {
  var replace_img, update_crop_params;

  update_crop_params = function(x, y, h, w) {
    $('#crop_param_x').val(x);
    $('#crop_param_y').val(y);
    $('#crop_param_height').val(h);
    return $('#crop_param_width').val(w);
  };

  replace_img = function(img_path) {
    return $('#entry_img_preview > img').cropper("replace", img_path);
  };

  $(function() {
    return $('#entry_img_preview > img').cropper({
      aspectRatio: 1,
      autoCropArea: 0.65,
      strict: false,
      guides: false,
      ...
      built: function() {
        ...
        return update_crop_params(params.x, params.y, params.height, params.width);
      },
      crop: function(e) {
        return update_crop_params(e.x, e.y, e.height, e.width);
      }
    });
  });

  $(function() {
    var fileInput, originalPath;
    fileInput = $('#image_input');
    originalPath = $('#entry_img_preview > img').attr('src');
    return fileInput.change(function() {
  ...
  $(function() {
    ...
    rules = {
      "contestant[contestant_profile_attributes][name]": {
        required: true
      },
      "contestant[contestant_profile_attributes][hurigana]": {
        required: true
      },
      "contestant[contestant_profile_attributes][come_from]": {
        required: true
      },
      "contestant[contestant_profile_attributes][height]": {
        required: true
      },
      "contestant[contestant_profile_attributes][comment]": {
        required: true
      },
      "contestant[contestant_profile_attributes][link_url]": {
        url: true
      },
      "contestant[contestant_profile_attributes][thanks_comment]": {
        required: true
      },
      "contestant[contestant_profile_attributes][profile_image]": {
        required: true
      },
      "contestant[email]": {
        required: true,
      ...
      "contestant[contestant_profile_attributes][is_share_with_twitter_ok]": {
        required: true
      },
      "contestant[contestant_profile_attributes][is_interest_in_idol_group]": {
        required: true
      },
    ...
    return $('#new_contestant').validate({
      rules: rules,
      messages: messages
    });
  });
